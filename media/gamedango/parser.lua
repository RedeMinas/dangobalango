...
-- Symbol used for dango.  This must appear only once in the level string.
local DANGO = 'c'
...
local function _parse (str)
   ...
   for i = 1, #lines do
      ...
      for j = 1, #lines[i] do
         ...
         -- Check for a single dango instance.
         if c == DANGO then
            assert (dango_found == false,
                    ('Dango (%s,%s) appears more than once')
                    :format (i, j))
            dango_found = true
         end
      end
   end
   assert (dango_found, 'Dango not found')
   return grid
end
