...
Display = {
   ...
   scoreboard = {
      pad_top = nil,
      height = 20,
      dango_live = canvas:new ('media/druzila.png'),
   },
...
do
   for _, dir in pairs (DIRS) do
      DANGO[dir] = canvas:new (ACTORS_PATHS[dir]:gsub('%%s', 'druzila', 1))
   end
end
local DANGO_DEAD = canvas:new ('media/dango_dead.png')
...
local function end_message (win)
   ...
   if win then
      ...
      canvas:attrFont('vera', 18)
      local text1 = 'VOCÊ GANHOU.'
     local text2 = 'PARABÉNS, '
         ..'OBRIGADO POR JOGAR.'
...
function Display:redraw (level, lives)
   ...
   -- Draw dango.
   local dango = self.board_ref:get_dango ()
   local dir = dango.dir
   if dir == nil or dir == 'NONE' then dir = 'EAST' end
   if dango.state.name == 'OK' then
      self:draw_anim (DANGO[dir], self.dango_frame,
                      dango.pos.i, dango.pos.j)
      self.dango_frame = (self.dango_frame + 1) % ANIMS_NFRAMES
   else
      self:draw_img (DANGO_DEAD, dango.pos.i, dango.pos.j)
   end
