...
local GHOST_STATES = {
   ...
   -- Ghost is fine; chase dango.
   ['OK'] = { name = 'OK' },

   -- Ghost is frozen; run-away from dango.
   ['FROZEN'] = {
      name = 'FROZEN',
      timer = 40,
   },
...
function ActorGhost:collisions ()
   ...
   local dango = self.board:get_dango ()

   -- Is colliding.
   if self.pos.i == dango.pos.i and self.pos.j == dango.pos.j then
      ...
      -- Kill dango.
         ...
         if dango.state.name == 'OK' then
            dango:set_state ('DYING')
         end
      end
   end
...
function ActorGhost:update_dir ()
   ...
   -- Run away from dango.
   if self.state.name == 'FROZEN' then
      if dango.pos.i > self.pos.i and dirs['NORTH'] then
         dir = 'NORTH'
      elseif dango.pos.i < self.pos.i and dirs['SOUTH'] then
         dir = 'SOUTH'
      elseif dango.pos.j > self.pos.j and dirs['WEST'] then
         dir = 'WEST'
      elseif dango.pos.j < self.pos.j and dirs['EAST'] then
         dir = 'EAST'
      end

   -- Chase him.
   else
      if dango.pos.i > self.pos.i and dirs['SOUTH'] then
         dir = 'SOUTH'
      elseif dango.pos.i < self.pos.i and dirs['NORTH'] then
         dir = 'NORTH'
      elseif dango.pos.j > self.pos.j and dirs['EAST'] then
         dir = 'EAST'
      elseif dango.pos.j < self.pos.j and dirs['WEST'] then
